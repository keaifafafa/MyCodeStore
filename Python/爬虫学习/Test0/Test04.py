...
url = 'http://www.duitang.com/napi/blog/list/by_search/?kw={}&start={}'
kw = '御姐'
kw = urllib.parse.quote(kw)
print(kw)

num = 1
for index in range(0, 128, 24):
    url = url.format(kw, index)
    response = requests.get(url)  # 浏览器
    we_data = response.text
    ...
    for i in photos:
        a = requests.get(i)
        with open(r'C:\Users\发发\Desktop\测试5\{}.jpg'.format(num), 'wb')as f:
            f.write(a.content)
            num += 1
